...
import { Users, Calendar, Receipt, UserCheck, FlaskConical, Pill, TrendingUp } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

export default function Dashboard() {
  const recentAppointments = mockAppointments.slice(0, 4);
    ...
    <MainLayout title="Dashboard">
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <StatCard
            title="Pending Bills"
            value={mockDashboardStats.pendingBills}
            icon={Receipt}
            variant="warning"
          />
          <StatCard
            title="Available Doctors"
            value={mockDashboardStats.availableDoctors}
            icon={UserCheck}
            variant="success"
          />
          <StatCard
            title="Pending Lab Tests"
            value={mockDashboardStats.pendingLabTests}
            icon={FlaskConical}
            variant="info"
          />
          <StatCard
            title="Low Stock Items"
            value={mockDashboardStats.lowStockMedicines}
            icon={Pill}
            variant="warning"
          />
        </div>
